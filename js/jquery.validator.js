...
(function($) {	
	...
	$.orpTools = $.orpTools || {version: '@VERSION'};
		
	// globals
	var typeRe = /\[type=([a-z]+)\]/, 
		numRe = /^-?[0-9]*(\.[0-9]+)?$/,
		dateInput = $.orpTools.dateinput,
	...
	v = $.orpTools.validator = {
      ...
      externalMessage: false,
		...
		/* The Error Messages */
		messages: {
			"*": { en: "You must fill out this field in order to proceed." }		
		},
	...
	var fns = [], effects = {
		...
		'default' : [
          ...
          var input_id = $(input).attr('data-id');					
            ...
            $('.qa' + input_id ).addClass('qa-error');
          
						msg = $(conf.message).addClass(conf.messageClass);
						input.data("msg.el", msg);
            //input.after(msg);
            
            //design for group validation
            $('.qa' + input_id ).append(msg);
					}  
				...
				//		msg.add(msg.find("p")).css({display: 'inline'});		
					...
					msg.css({ visibility: 'visible', 'positon': 'absolute' }).fadeIn(conf.speed);   
        ...
        var id = $(inputs).attr('data-id');
        $('.qa' + id).removeClass('qa-error');
        $('.qa' + id).remove('.error');        
        $('.error').remove();
			}
		]  
  ...
  v.fn("#email", "Please enter a valid email address.", function(el, v) {		 			
	...
	v.fn("[required]", "You must check out this field in order to proceed.", function(el, v) {
		if (el.is(":checkbox")) { 
	...
	/*v.fn(":checkbox", "Please check an option.", function(el) {
		var	checked = false;
		var	els = $("[name='" + el.attr("name") + "']").each(function(i, el) {
			if ($(el).is(":checked")) {
				checked = true;
	...
	$.fn.orpValidator = function(conf) { 
		...
		if (this.is("form")) {
			return this.each(function() {			
				var form = $(this); 
				instance = new Validator(form.find(":input[required]"), form, conf);	 
